...
import { CacheService, createCacheService } from '../db/cache'
import { ChatsCache, createChatsCache } from '../db/chats-cache'
import { createTestCacheDatabase } from '../db/schema'
import type { CachedEntity, ChatType, DurationString } from '../db/types'
...
describe('CacheService', () => {
  ...
  beforeEach(() => {
    ...
    db = testDb.db
    cacheService = createCacheService(db)
  })

  describe('configuration', () => {
    it('should use default config when not provided', () => {
      const config = cacheService.getConfig()
      expect(config.staleness.peers).toBe(7 * 24 * 60 * 60 * 1000) // 1 week
      expect(config.staleness.dialogs).toBe(60 * 60 * 1000) // 1 hour
    ...
    it('should update config', () => {
      ...
      cacheService.setConfig(newConfig)
      expect(cacheService.getConfig()).toEqual(newConfig)
    })
  })
  ...
  describe('isStale', () => {
    it('should use default TTL when not specified', () => {
      ...
      expect(cacheService.isStale(fresh)).toBe(false)
    })

    it('should use custom TTL when specified', () => {
      const fresh = Date.now() - 5000 // 5 seconds ago
      expect(cacheService.isStale(fresh, 10000)).toBe(false) // 10s TTL
      expect(cacheService.isStale(fresh, 1000)).toBe(true) // 1s TTL
    })
  })
  ...
  describe('basic CRUD', () => {
    ...
    it('deletes entries', () => {
      ...
      expect(cacheService.delete('users_cache', 'user_id', '1')).toBe(true)
      expect(cacheService.delete('users_cache', 'user_id', '1')).toBe(false)
    })
  })

  describe('stale/prune/stats', () => {
    it('returns stale entries and prunes old data', () => {
      const now = Date.now()
      cacheService.set<UserEntity>('users_cache', 'user_id', '1', {
        username: 'fresh',
        fetched_at: now,
        raw_json: '{}',
        is_contact: 0,
      ...
      cacheService.set<UserEntity>('users_cache', 'user_id', '2', {
        username: 'stale',
      ...
      const stale = cacheService.getStaleEntries<UserEntity>(
        'users_cache',
        5_000,
      )
      ...
      const pruned = cacheService.pruneOldEntries('users_cache', 5_000)
      expect(pruned).toBe(1)
    })

    it('returns stats with custom TTL', () => {
      const now = Date.now()
      cacheService.set<UserEntity>('users_cache', 'user_id', '1', {
        username: 'fresh',
        fetched_at: now,
      ...
      const stats = cacheService.getStats('users_cache', 5_000)
      expect(stats.total).toBe(2)
      expect(stats.stale).toBe(1)
      expect(stats.oldest).toBeLessThan(now)
    })
  })

  describe('getMany/getAll/clear', () => {
    it('returns many entries by key', () => {
      cacheService.set<UserEntity>('users_cache', 'user_id', '1', {
        username: 'one',
        fetched_at: Date.now(),
        raw_json: '{}',
      ...
      expect(
        cacheService.getMany<UserEntity>('users_cache', 'user_id', []).length,
      ).toBe(0)

      const rows = cacheService.getMany<UserEntity>('users_cache', 'user_id', [
        '2',
        '1',
      ])
    ...
    it('returns entries with limit and offset', () => {
      cacheService.set<UserEntity>('users_cache', 'user_id', '1', {
        username: 'one',
        fetched_at: Date.now() - 5000,
        raw_json: '{}',
        is_contact: 0,
        is_bot: 0,
        is_premium: 0,
      ...
      const rows = cacheService.getAll<UserEntity>('users_cache', {
    ...
    it('clears all entries', () => {
      ...
      expect(cacheService.clear('users_cache')).toBe(1)
      expect(cacheService.getAll('users_cache', { limit: 10 }).length).toBe(0)
    })
  })
})
...
describe('Factory Functions', () => {
  ...
  describe('createCacheService', () => {
    it('should create CacheService instance', () => {
      const { db } = createTestCacheDatabase()
      const service = createCacheService(db)
